...
// CanReviewBug — кто вправе подтверждать и отклонять отчёты о багах.
// Проверка — работа того, кто баги чинит, поэтому круг тот же, что у
// заведения задач: наблюдатель отчёты читает, но решений не принимает.
func (r Role) CanReviewBug() bool {
	return r.CanCreateTask()
}
